...
exercised by a real delivery in this repo — validate against this spec on
first use.)
...
- Phase 9 — an in-diff pre-PR-gate failure that can't converge: open the PR
...
needs. The entry points here are a **scheduled cloud agent** (the `/schedule`
skill) or a `ScheduleWakeup`-resumed session whose prompt is
`/deliver auto …` — there is no cron/headless GitHub-Actions trigger in this
repo.
...
- **User-scoped MCP may be absent** (`mcp__github__*`, `wiki`,
  `mcp__statsig__*`, the `xcode` MCP). The `gh` fallbacks in `/pr` and
  `/watch-pr` cover GitHub; the build/test skills already fall back to
  `make` via Haiku subagents; the wiki step degrades silently. A missing
  **Statsig** MCP blocks feature-flag gate creation (Phase 3 bookkeeping) —
  note it in the PR description as a follow-up rather than stopping.

**Recommendation — don't routinise async *feature* delivery here.** This is a
single-maintainer app where the ready-to-merge human gate is deliberate —
every change ships to Adam's own devices via TestFlight/App Store, so it is
worth a human's eyes. Async earns its place for the *occasional*
away-from-keyboard run — not as the default path.
