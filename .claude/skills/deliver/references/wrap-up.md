...
Every push to the PR branch re-triggers the `claude-review` job (which
re-reviews **statelessly**, so each push can spawn a fresh batch of threads —
see the auto-memory's *claude-review loop* notes) and the CI matrix. When the
retro was a routine post-gate push, every delivery paid a re-review + re-run +
re-watch for a markdown file — and on the sibling TMDb project one such push
raised a High thread that blocked the merge. Writing the retro pre-PR makes
the default post-gate push count **zero**; the re-watch rule survives only for
the exceptions (a `watch:` amendment, an approved skill edit from the scan).
...
- **Degrade silently if the `wiki` MCP is absent** (a headless/scheduled
  run) — never block on it.
...
1. **Read the recent window + the log + the pipeline skills.** The **~last
   12** entries of `knowledge/delivery-retros.md` (older ones are archived
   one-liners, so this is the whole live history), **all** of
   `knowledge/skill-improvement-log.md`, and every `SKILL.md` for the
   **pipeline skills and the sub-skills they invoke** (`deliver`,
   `review-plan`, `implement-plan`, `canon-tdd`, `review-changes`,
   `capture-knowledge`, `pr`, `watch-pr`, `fix-pr-checks`,
   `diagnose-ci-failure`, plus `/test-snapshots`, `/build-package`,
   `/test-package`, and the `swift-concurrency` / `swift-testing-expert` /
   `swiftui-expert` / `swiftdata-expert` / `snapshot-testing-expert` /
   `documentation-writer` agents the pipeline pulls in). Skip the unrelated
   domain skills (`add-context`, `create-domain-model`, …) — a delivery retro
   won't drive a change there. The bounded retro read keeps the scan's cost
