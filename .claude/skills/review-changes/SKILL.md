...
const DIMENSIONS = [
  ...
  { key: 'swiftui', focus: 'SwiftUI — foregroundStyle (not foregroundColor), clipShape(.rect(cornerRadius:)) (not cornerRadius), Tab API, NavigationStack + navigationDestination(for:); NO hard-coded spacing (use .spacing4/8/12/16); no ObservableObject/AnyView/hard-coded font sizes; localization bundle:.module + SCREAMING_SNAKE_CASE keys; accessibility labels/traits' },
  { key: 'swiftdata', focus: 'SwiftData / CloudKit — no @Attribute(.unique) on CloudKit-synced models; properties & relationships optional-or-defaulted in synced stores; @Model never exposed outside Infrastructure; a new VersionedSchema version + MigrationStage added when CloudKit @Model classes change' },
  { key: 'testing', focus: 'tests at ALL layers — adapter mappers, use cases, AND view models; Swift Testing with try #require (not force-unwrap); error paths and edge cases (boundaries / empty / nil), not just the happy path; feature-flag tests updated for both enabled and disabled paths; new unit-test targets registered in TestPlans/PopcornUnitTests.xctestplan (snapshot targets in PopcornSnapshotTests.xctestplan)' },
  { key: 'project-specific', focus: 'project-specific rules — TMDb mapping compliance (docs/TMDB_MAPPING.md 4-layer pipeline, naming, nil handling); remote TMDb calls use language: nil (NOT "en"); a Statsig gate exists / is created for any new FeatureFlag (gate ID = FeatureFlag.id, snake_case); no hard-coded locale/region/language values' },
]
...
const reviewPrompt = (d) =>
  ...
  `Follow the project review guidelines in ${SPEC} — the severity rubric, the review dimensions, and the MANDATORY adversarial re-evaluation (drop any finding that doesn't survive) — and read the relevant docs/*.md (SWIFT, SWIFTUI, SWIFTDATA, ARCHITECTURE, TMDB_MAPPING) it points to as the source of truth. ` +
...
const verified = await parallel(blocking.map((f) => () =>
  agent(
    ...
    `Try to REFUTE it. Read the actual code (\`git diff ${BASE}...HEAD\`, then open the file) and decide whether the issue is REAL and in scope for THIS change, per ${SPEC} and the docs/*.md it points to. ` +
