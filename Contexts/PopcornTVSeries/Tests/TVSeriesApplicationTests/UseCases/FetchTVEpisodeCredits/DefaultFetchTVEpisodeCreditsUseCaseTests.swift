//
//  DefaultFetchTVEpisodeCreditsUseCaseTests.swift
//  PopcornTVSeries
//
...
@Suite("DefaultFetchTVEpisodeCreditsUseCase")
struct DefaultFetchTVEpisodeCreditsUseCaseTests {

    let mockRepository: MockTVEpisodeCreditsRepository
    let mockAppConfigurationProvider: MockAppConfigurationProvider

    init() {
        self.mockRepository = MockTVEpisodeCreditsRepository()
    ...
    func executeReturnsCreditsDetailsOnSuccess() async throws {
        ...
        let result = try await useCase.execute(tvSeriesID: 123, seasonNumber: 1, episodeNumber: 1)
    ...
    @Test("execute passes correct parameters to repository")
    func executePassesCorrectParametersToRepository() async throws {
        mockRepository.creditsStub = .success(.mock())
        mockAppConfigurationProvider.appConfigurationStub = .success(.mock())

        let useCase = makeUseCase()

        _ = try await useCase.execute(tvSeriesID: 456, seasonNumber: 2, episodeNumber: 3)

        #expect(mockRepository.creditsCallCount == 1)
        #expect(mockRepository.creditsCalledWith[0].tvSeriesID == 456)
        #expect(mockRepository.creditsCalledWith[0].seasonNumber == 2)
        #expect(mockRepository.creditsCalledWith[0].episodeNumber == 3)
    }
    ...
    func executeThrowsNotFoundWhenRepositoryThrowsNotFound() async {
        ...
        await #expect(
            performing: {
                try await useCase.execute(tvSeriesID: 123, seasonNumber: 1, episodeNumber: 1)
            },
            throws: { error in
                guard let creditsError = error as? FetchTVEpisodeCreditsError else {
                    return false
                }
    ...
    private func makeUseCase() -> DefaultFetchTVEpisodeCreditsUseCase {
        DefaultFetchTVEpisodeCreditsUseCase(
            tvEpisodeCreditsRepository: mockRepository,
