//
//  DefaultTVEpisodeCreditsRepositoryTests.swift
//  PopcornTVSeries
//
...
@Suite("DefaultTVEpisodeCreditsRepository")
struct DefaultTVEpisodeCreditsRepositoryTests {

    let mockRemoteDataSource: MockTVEpisodeRemoteDataSource
    let mockLocalDataSource: MockTVEpisodeCreditsLocalDataSource

    init() {
        self.mockRemoteDataSource = MockTVEpisodeRemoteDataSource()
        self.mockLocalDataSource = MockTVEpisodeCreditsLocalDataSource()
    }
    ...
    func creditsReturnsCachedValueWhenAvailable() async throws {
        ...
        let result = try await repository.credits(forEpisode: 1, inSeason: 1, inTVSeries: 123)
    ...
    func creditsFetchesFromRemoteWhenCacheIsEmpty() async throws {
        ...
        let result = try await repository.credits(forEpisode: 1, inSeason: 1, inTVSeries: 456)
    ...
    func creditsCachesRemoteValueAfterFetching() async throws {
        ...
        _ = try await repository.credits(forEpisode: 3, inSeason: 2, inTVSeries: 789)
        ...
        #expect(mockLocalDataSource.setCreditsCalledWith[0].episodeNumber == 3)
        #expect(mockLocalDataSource.setCreditsCalledWith[0].seasonNumber == 2)
        #expect(mockLocalDataSource.setCreditsCalledWith[0].tvSeriesID == 789)
    }

    @Test("credits passes correct parameters to data sources")
    func creditsPassesCorrectParametersToDataSources() async throws {
        mockLocalDataSource.creditsStub = .success(nil)
        ...
        _ = try await repository.credits(forEpisode: 5, inSeason: 3, inTVSeries: 999)

        #expect(mockLocalDataSource.creditsCalledWith[0].episodeNumber == 5)
        #expect(mockLocalDataSource.creditsCalledWith[0].seasonNumber == 3)
        #expect(mockLocalDataSource.creditsCalledWith[0].tvSeriesID == 999)
        #expect(mockRemoteDataSource.creditsCalledWith[0].episodeNumber == 5)
        #expect(mockRemoteDataSource.creditsCalledWith[0].seasonNumber == 3)
        #expect(mockRemoteDataSource.creditsCalledWith[0].tvSeriesID == 999)
    }
    ...
    func creditsThrowsNotFoundWhenRemoteThrowsNotFound() async {
        ...
        await #expect(
            performing: {
                try await repository.credits(forEpisode: 1, inSeason: 1, inTVSeries: 123)
            },
            throws: { error in
                guard let repoError = error as? TVEpisodeCreditsRepositoryError else {
                    return false
                }
    ...
    func creditsReturnsRemoteDataWhenCacheWriteFails() async throws {
        ...
        let result = try await repository.credits(forEpisode: 1, inSeason: 1, inTVSeries: 321)
    ...
    private func makeRepository() -> DefaultTVEpisodeCreditsRepository {
        DefaultTVEpisodeCreditsRepository(
