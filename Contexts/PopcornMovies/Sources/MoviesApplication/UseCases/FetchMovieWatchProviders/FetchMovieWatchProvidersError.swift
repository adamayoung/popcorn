//
//  FetchMovieWatchProvidersError.swift
//  PopcornMovies
//
...
///
/// Errors that can occur when fetching a movie's watch providers.
///
/// This error type represents all possible failure scenarios when attempting
/// to retrieve watch provider information through ``FetchMovieWatchProvidersUseCase``.
/// Errors from underlying repositories are mapped to this type for consistent
/// error handling at the application layer.
///
public enum FetchMovieWatchProvidersError: Error {
...
extension FetchMovieWatchProvidersError {

    init(_ error: Error) {
        if let repositoryError = error as? MovieWatchProvidersRepositoryError {
    ...
    init(_ error: MovieWatchProvidersRepositoryError) {
        switch error {
        case .notFound:
            self = .notFound
        case .unauthorised:
