...
///
/// Errors that can occur when streaming popular movies.
///
/// This error type represents all possible failure scenarios when attempting
/// to stream or paginate popular movies through ``StreamPopularMoviesUseCase``.
///
...
extension StreamPopularMoviesError {
    ...
    ///
    /// Creates a stream popular movies error from any error type.
    ///
    /// This initializer maps known error types from repositories to the appropriate
    /// ``StreamPopularMoviesError`` case. Unrecognised errors are wrapped
    ...
    ///
    /// Creates a stream popular movies error from a movie repository error.
    ///
