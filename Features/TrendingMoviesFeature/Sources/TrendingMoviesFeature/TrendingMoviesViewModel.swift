...
public final class TrendingMoviesViewModel {
    ...
    /// contain only already-seen movies. TMDb's trending feed shifts between
    ...
    /// Whether at least one more page of trending movies is available to load.
    ...
    public func load() async {
        ...
        do {
            page = try await dependencies.fetchTrendingMovies(1)
        } catch {
    ...
    /// Fetches and appends the next page of trending movies.
    ///
    ...
    /// de-duplicated by id (TMDb's trending feed shifts between page requests, so
    ...
    public func loadMore() async {
        ...
        do {
            for _ in 0 ..< Self.maxLoadMoreAttempts {
                ...
                Self.logger.info("User fetching trending movies page \(self.currentPage + 1)")
                let page = try await dependencies.fetchTrendingMovies(currentPage + 1)
                currentPage = page.page
                hasMore = page.hasMore
            ...
            if !Task.isCancelled, !(error is CancellationError) {
                Self.logger.error("Failed fetching more trending movies: \(error, privacy: .public)")
            }
        }
    }
