...
@Suite("DiscoverMoviesViewModel Tests")
struct DiscoverMoviesViewModelTests {
    ...
    func loadSuccessBuildsReadySnapshot() async {
        let viewModel = Support.makeViewModel(
            dependencies: Support.stubDependencies(
                fetchDiscoverMovies: { _ in Support.singlePage(Support.testMovies) }
            )
        )
    ...
    func loadSuccessWithNoMoviesIsReady() async {
        let viewModel = Support.makeViewModel(
            dependencies: Support.stubDependencies(
                fetchDiscoverMovies: { _ in Support.singlePage([]) }
            )
        )
    ...
    func loadNoOpWhenReady() async {
        ...
        let viewModel = Support.makeViewModel(
            ...
            viewState: .ready(DiscoverMoviesViewSnapshot(movies: Support.testMovies))
        )
    ...
    func loadCancellationResetsToInitial() async {
        let viewModel = Support.makeViewModel(
            dependencies: Support.stubDependencies(
                fetchDiscoverMovies: { _ in throw CancellationError() }
            )
        )
    ...
    func selectMovieInvokesNavigator() {
        let navigator = SpyDiscoverMoviesNavigator()
        let viewModel = Support.makeViewModel(navigator: navigator)

        viewModel.selectMovie(id: 456, transitionID: "456_discover-movies-grid")

        #expect(navigator.openedMovieID == 456)
        #expect(navigator.openedMovieTransitionID == "456_discover-movies-grid")
    }
