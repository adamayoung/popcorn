//
//  MoviePreviewPage.swift
//  DiscoverMoviesFeature
//
//  Copyright © 2026 Adam Young.
//

import Foundation

/// A single page of discover movie previews for presentation.
///
/// Carries the ``MoviePreview`` values on one page together with the pagination
/// metadata ``DiscoverMoviesViewModel`` needs to decide whether to request another
/// page.
public struct MoviePreviewPage: Equatable, Sendable {

    /// The number of this page, starting at `1`.
    public let page: Int

    /// The total number of pages available.
    public let totalPages: Int

    /// The discover movie previews on this page.
    public let movies: [MoviePreview]

    /// Whether at least one more page is available beyond this one.
    public var hasMore: Bool {
        page < totalPages
    }

    /// Creates a new page of discover movie previews.
    ///
    /// - Parameters:
    ///   - page: The number of this page, starting at `1`.
    ///   - totalPages: The total number of pages available.
    ///   - movies: The discover movie previews on this page.
    public init(page: Int, totalPages: Int, movies: [MoviePreview]) {
        self.page = page
        self.totalPages = totalPages
        self.movies = movies
    }

}
