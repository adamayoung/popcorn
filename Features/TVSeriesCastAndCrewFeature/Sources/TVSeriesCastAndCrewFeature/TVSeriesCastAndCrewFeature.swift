//
//  TVSeriesCastAndCrewFeature.swift
//  Popcorn
//
...
@Reducer
public struct TVSeriesCastAndCrewFeature: Sendable {
...
extension TVSeriesCastAndCrewFeature {

    private func handleFetch(_ state: inout State) -> EffectOf<Self> {
        .run { [state, client] send in
            Self.logger.info(
                "Fetching cast and crew [tvSeriesID: \(state.tvSeriesID, privacy: .private)]"
            )

            let credits: Credits
            do {
                credits = try await client.fetchCredits(tvSeriesID: state.tvSeriesID)
            } catch {
                Self.logger.error(
                    "Failed fetching cast and crew [tvSeriesID: \(state.tvSeriesID, privacy: .private)]: \(error.localizedDescription, privacy: .public)"
                )
