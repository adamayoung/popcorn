...
/// A pushed destination on the developer menu's navigation stack.
enum DeveloperRoute: Hashable {
    case featureFlags
}

/// The developer menu, presented as a sheet from the app root (DEBUG only).
///
/// Hosts a static home list in a `NavigationStack` and pushes the feature-flags
/// screen. The App layer injects ``makeFeatureFlags`` so the screen can build its
/// child view model from the shared services without depending on the composition
/// layer.
